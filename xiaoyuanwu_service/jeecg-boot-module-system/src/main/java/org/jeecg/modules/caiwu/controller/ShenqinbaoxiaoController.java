...
@Api(tags = "报销")
@RestController
@RequestMapping("/caiwu/shenqinbaoxiao")
@Slf4j
public class ShenqinbaoxiaoController extends JeecgController<BaoXiao, IBaoXiaoService> {
    @Autowired
    ...
    public Result<?> add(@RequestBody BaoXiao baoXiao) {
        LoginUser loginUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
        baoXiao.setUserId(loginUser.getId());
        baoXiao.setUserName(loginUser.getRealname());
